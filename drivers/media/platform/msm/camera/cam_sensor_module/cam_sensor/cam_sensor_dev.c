...
struct cam_sensor_settings {
	...
	struct cam_sensor_i2c_reg_setting_array s5khm2s_setting;
};
	...
	unsigned int cmd, void *arg)
{
	...
	case VIDIOC_CAM_FTM_POWNER_UP:
        ...
        } else if(s_ctrl->sensordata->slave_info.sensor_id == 0x1ad2 ||
                    s_ctrl->sensordata->slave_info.sensor_id == 0x1ad3) {
            sensor_setting.reg_setting = sensor_settings.s5khm2s_setting.reg_setting;
            sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
            sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
            sensor_setting.size = sensor_settings.s5khm2s_setting.size;
            sensor_setting.delay = sensor_settings.s5khm2s_setting.delay;
            CAM_ERR(CAM_SENSOR,"FTM GET S5KHM2S setting");
        }
